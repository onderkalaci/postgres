...
# which index to use
$node_publisher->safe_psql('postgres',
	"UPDATE test_replica_id_full SET y = y + 1 WHERE y = 30;");
$node_publisher->wait_for_catchup($appname);
...
#
# Basic test where the subscriber uses index and updates/deletes 20 rows
#
...
$node_publisher->safe_psql('postgres',
	"INSERT INTO test_replica_id_full SELECT (i%10), (i%10)::text FROM generate_series(0,100)i;"
);
...
# wait until the index is used on the subscriber
$node_subscriber->poll_query_until(
	'postgres', q{select (idx_scan = 40) from pg_stat_all_indexes where indexrelname = 'test_replica_id_full_idx';}
) or die "Timed out while waiting for check subscriber tap_sub_rep_full updates 20 rows via index";

# make sure that the subscriber has the correct data after the DELETE
...
#
# Basic test where the subscriber uses index and updates multiple rows with a
# table that has dropped columns
#
...
# wait until the index is used on the subscriber
$node_subscriber->poll_query_until(
	'postgres', q{select (idx_scan = 20) from pg_stat_all_indexes where indexrelname = 'test_replica_id_full_idx';}
) or die "Timed out while waiting for check subscriber tap_sub_rep_full updates 20 rows via index";

# make sure that the subscriber has the correct data
$result = $node_subscriber->safe_psql('postgres',
	"select sum(x+y::int) from test_replica_id_full;");
is($result, qq(920), 'ensure subscriber has the correct data at the end of the test');
...
# wait until the index is used on the subscriber
$node_subscriber->poll_query_until(
	'postgres', q{select sum(idx_scan)=1 from pg_stat_all_indexes where indexrelname ilike 'users_table_part_%';}
...
$result = $node_subscriber->safe_psql('postgres',
	"SELECT count(*) FROM people;");
is($result, qq(19), 'ensure subscriber has the correct data at the end of the test');
...
# now, create a unique index and set the replica identity
$node_publisher->safe_psql('postgres',
...
# =============================================================================
# Testcase start: PUBLICATION LACKS THE COLUMN ON THE SUBS INDEX
#
...
# now, we update only 1 row on the publisher and expect
# the subscriber to only update 1 row via the index
$node_publisher->safe_psql('postgres',
	"UPDATE test_replica_id_full SET x = x + 1 WHERE x = 15;");
$node_publisher->wait_for_catchup($appname);
...
# Testcase end: PUBLICATION LACKS THE COLUMN ON THE SUBS INDEX
# =============================================================================
